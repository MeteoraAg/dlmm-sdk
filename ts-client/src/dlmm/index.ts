...
export class DLMM {
  ...
  public async getLbPairLockInfo(
    ...
    const [lbPairPositionsV2, lbPairPositionsV3] = await Promise.all([
      this.program.account.positionV2.all([positionLbPairFilter(this.pubkey)]),
      this.program.provider.connection.getProgramAccounts(
        this.program.programId,
        {
          filters: [positionV3DiscFilter(), positionLbPairFilter(this.pubkey)],
        }
      ),
    ]);

    const lbPairPositions = [
      ...lbPairPositionsV2.map(
        (p) => new PositionV2Wrapper(p.publicKey, p.account)
      ),
      ...lbPairPositionsV3.map((p) =>
    ...
    const positions = positionsWithLock;
  ...
  public async createEmptyPosition({
    ...
    const instructions = [...createBinArrayIxs, createPositionIx];
  ...
  public async removeLiquidity({
    ...
    for (const { lowerBinId, upperBinId } of chunkedBinRange) {
      ...
      if (
        ...
        // TODO: idempotent else have to be sequential
        const closeWrappedSOLIx = await unwrapSOLInstruction(user);
        closeWrappedSOLIx && postInstructions.push(closeWrappedSOLIx);
      }
  ...
  /**
   * Migrates a position to v3.
   *
   * The given positionV2 | position must exist and be owned by the current user.
   * The given positionV3 must not exist.
   *
   * The given fee payer will be used to pay for the transaction.
   *
   * After this function is called, the positionV2 is no longer valid,
   * and the positionV3 is valid.
   *
   * @param posiiton The public key of the v1 / v2 position to migrate.
   * @param positionV3 The public key of the v3 position to create.
   * @param feePayer The public key of the account to pay for the transaction.
   * @returns A transaction that migrates the position from v2 to v3.
   */
  public async migratePosition({
    position,
    positionV3,
    feePayer,
  }: {
    ...
    const positionAccount =
      await this.program.provider.connection.getAccountInfo(position);
    const disc = positionAccount.data.subarray(0, 8);

    let migratePositionIx;

    const getLowerAndUpperBinArray = (lowerBinId: number) => {
      const lowerBinArrayIndex = binIdToBinArrayIndex(new BN(lowerBinId));
      const upperBinArrayIndex = lowerBinArrayIndex.addn(1);

      return [lowerBinArrayIndex, upperBinArrayIndex].map((index) => {
        return deriveBinArray(this.pubkey, index, this.program.programId)[0];
      });
    };

    if (disc.equals(POSITION_V2_DISC)) {
      const positionState: PositionV2 = this.program.coder.accounts.decode(
        this.program.account.positionV2.idlAccount.name,
        positionAccount.data
      );

      migratePositionIx = await this.program.methods
        ...
        .instruction();
    } else if (disc.equals(POSITION_V1_DISC)) {
      const positionState: Position = this.program.coder.accounts.decode(
        this.program.account.position.idlAccount.name,
        positionAccount.data
      );

      const [binArrayLower, binArrayUpper] = getLowerAndUpperBinArray(
      ...
      // Unreachable, but for sanity check
      throw new Error("Invalid position version");
    }
