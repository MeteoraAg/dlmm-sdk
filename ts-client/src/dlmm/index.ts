...
export class DLMM {
	...
	static async create(
		...
		const tokenXDecimal = MintLayout.decode(reserveAccountsInfo[2].data).decimals
		const tokenYDecimal = MintLayout.decode(reserveAccountsInfo[3].data).decimals
		const tokenX = {
	...
	static async createMultiple(
		...
		const binArrayBitMapExtensionsMap = new Map<string, BinArrayBitmapExtension>()
		...
		const reserveAndTokenMintAccountsInfo = await chunkedGetMultipleAccountInfos(
			program.provider.connection,
			[...reservePublicKeys, ...tokenMintPublicKeys]
		)
		...
		const lbClmmImpl = await Promise.all(
			dlmmList.map(async (lbPair, index) => {
				...
				const tokenYMintAccountInfo =
					reserveAndTokenMintAccountsInfo[reservePublicKeys.length + index * 2 + 1]
				...
				const tokenXDecimal = MintLayout.decode(tokenXMintAccountInfo.data).decimals
				const tokenYDecimal = MintLayout.decode(tokenYMintAccountInfo.data).decimals
				const tokenX = {
	...
	static async getAllLbPairPositionsByUser(
		...
		const [clockAccInfo, ...binArraysAccInfo] = await chunkedGetMultipleAccountInfos(
			connection,
			[SYSVAR_CLOCK_PUBKEY, ...binArrayPubkeyArrayV2, ...lbPairArrayV2]
		)
	...
	public async getLbPairLockInfo(lockDurationOpt?: number): Promise<PairLockInfo> {
		...
		const clockAccInfo =
			await this.program.provider.connection.getAccountInfo(SYSVAR_CLOCK_PUBKEY)
	...
	public async refetchStates(): Promise<void> {
		...
		const [tokenXDecimal, tokenYDecimal] = await Promise.all([
			getTokenDecimals(this.program.provider.connection, lbPairState.tokenXMint),
			getTokenDecimals(this.program.provider.connection, lbPairState.tokenYMint)
		])
	 ...
	 */
	public async getBinArrayForSwap(swapForY, count = 4): Promise<BinArrayAccount[]> {
	...
	public getDynamicFee(): Decimal {
		...
		const totalFee = getTotalFee(this.lbPair.binStep, sParameters, vParameterClone)
	...
	public async initializePositionAndAddLiquidityByStrategy({
		...
		if (
			[this.tokenX.publicKey.toBase58(), this.tokenY.publicKey.toBase58()].includes(
		...
		const instructions = [...preInstructions, addLiquidityIx, ...postInstructions]
	...
	public async removeLiquidity({
		...
		const instructions = [...preInstructions, removeLiquidityTx, ...postInstructions]
	...
	public swapQuoteExactOut(
		...
		const endPrice = getPriceOfBinByBinId(activeId.toNumber(), this.lbPair.binStep)
	 ...
	 *    - `operator`: Operator of the position
	 *    - `lockReleasePoint`: Timestamp / slot at which the position can withdraw the liquidity
	...
	public async seedLiquidity(
		...
		for (let i = 0; i < positionCount.toNumber(); i++) {
			...
			const {
				...
				lowerBinArrayState,
				upperBinArrayState,
				instructions: initPositionAndBinArrayIxs
			} = await this.preparePositionAndBinArrayForSeeding(
				lowerBinId,
				upperBinId,
				base,
				positionPda,
				...
				owner,
				feeOwner,
				appendedInitBinArrayIx,
				lockReleasePoint
			)

			if (initPositionAndBinArrayIxs.length > 0) {
			...
			const {
				simAmountDeposited,
				simLossAmountDeposited,
				simTotalLossAmount,
				binLiquidityDistribution
			} = this.simulateAndPreparePositionDepositParams(
				positionSeedAmount,
				lowerBinId.toNumber(),
				cappedUpperBinId,
				binDepositAmount
			...
			let amountXInPositionRangeOnChain = this.getSeededAmountXInBinRange(
				lowerBinArrayState,
				upperBinArrayState,
				lowerBinId.toNumber(),
				cappedUpperBinId
			...
			let simPositionAmountX = amountXInPositionRangeOnChain
			...
			// Take care of 4 scenarios:
			// 1. Success deposit liquidity but fail deposit loss
			// 2. Success deposit loss but fail deposit liquidity
			// 3. Both success
			// 4. Both fail

			// Position empty / loss amount deposited
			if (
				simPositionAmountX.isZero() ||
				simPositionAmountX.eq(simLossAmountDeposited)
			) {
				...
				simPositionAmountX = simPositionAmountX.add(simAmountDeposited)
			}

			// Position deposited initial liquidity, redeposit loss
			if (simPositionAmountX.eq(simAmountDeposited)) {
				addLiquidityIxs.push([
					ComputeBudgetProgram.setComputeUnitLimit({
						units: DEFAULT_ADD_LIQUIDITY_CU
					}),
				...
				simPositionAmountX = simPositionAmountX.add(simLossAmountDeposited)
			}

			simTotalAmountXOnChain = simTotalAmountXOnChain.add(simPositionAmountX)

			// Last position for final loss deposit
			if (
				i == positionCount.toNumber() - 1 &&
				!seedAmount.eq(simTotalAmountXOnChain)
			) {
				addLiquidityIxs.push([
					ComputeBudgetProgram.setComputeUnitLimit({
						units: DEFAULT_ADD_LIQUIDITY_CU
					...
					await this.program.methods
						.addLiquidity({
							amountX: seedAmount.sub(simTotalAmountXOnChain),
							amountY: new BN(0),
							binLiquidityDist: [
								{
									binId: cappedUpperBinId,
									distributionX: 10_000,
									distributionY: 0
								}
							]
						})
	...
	public async seedLiquiditySingleBin(
		...
		if (shouldSeedPositionOwner) {
			const positionOwnerTokenXAccount =
				await this.program.provider.connection.getAccountInfo(positionOwnerTokenX)
	...
	public async initializeBinArrays(binArrayIndexes: BN[], funder: PublicKey) {
		...
		for (const idx of binArrayIndexes) {
			const [binArray] = deriveBinArray(this.pubkey, idx, this.program.programId)
	...
	public async claimAllRewards({
		...
		const createATAAccAndIx = await Promise.all(
			tokensInvolved
				.map((token) => {
					...
					//
					return [
						getOrCreateATAInstruction(this.program.provider.connection, token, owner)
					]
				})
				.flat()
		)
		...
		return Promise.all(
			chunkedClaimAllTx.map(async (claimAllTx) => {
				...
				const instructions = [...preInstructions, ...mainIxs, ...postInstructions]
	...
	public canSyncWithMarketPrice(marketPrice: number, activeBinId: number) {
		...
		const marketPriceBinArrayIndex = binIdToBinArrayIndex(new BN(marketPriceBinId))
	...
	public async syncWithMarketPrice(marketPrice: number, owner: PublicKey) {
		...
		const binArrayAccounts =
			await this.program.provider.connection.getMultipleAccountsInfo(accountsToFetch)
	...
	private static async getClaimableLMReward(
		...
		if (!lowerBinArray || !upperBinArray) {
			...
			;[_lowerBinArray, _upperBinArray] =
				await program.account.binArray.fetchMultiple([lowerBinArray, upperBinArray])
		}

		if (!_lowerBinArray || !_upperBinArray) throw new Error("BinArray not found")
	...
	private async binArraysToBeCreate(lowerBinArrayIndex: BN, upperBinArrayIndex: BN) {
	...
	private async createClaimSwapFeeMethod({
		...
		const walletToReceiveFee = feeOwner.equals(PublicKey.default) ? owner : feeOwner
	...
	private getSeedAmountForPosition(
		lowerBinId: number,
		upperBinId: number,
		binDepositAmount: Map<number, BN>
	) {
		let totalAmount = new BN(0)

		for (let i = lowerBinId; i <= upperBinId; i++) {
			const amount = binDepositAmount.get(i)
	...
	private getSeededAmountXInBinRange(
		lowerBinArrayState: BinArray | null,
		upperBinArrayState: BinArray | null,
		lowerBinId: number,
		upperBinId: number
	) {
		let totalAmount = new BN(0)

		for (const binArrayState of [lowerBinArrayState, upperBinArrayState].filter(
			Boolean
		)) {
			const [binArrayLowerBinId, binArrayUpperBinId] = getBinArrayLowerUpperBinId(
				binArrayState.index
			)

			for (
				let binId = binArrayLowerBinId.toNumber();
				binId <= binArrayUpperBinId.toNumber();
				binId++
			) {
				if (binId >= lowerBinId && binId <= upperBinId) {
					const bin = binArrayState.bins[binId - binArrayLowerBinId.toNumber()]
					totalAmount = totalAmount.add(bin.amountX)
				}
			}
		}
	...
	private simulateAndPreparePositionDepositParams(
		positionSeedAmount: BN,
		lowerBinId: number,
		upperBinId: number,
		binDepositAmount: Map<number, BN>
	) {
		let simAmountDeposited = new BN(0)
		let simTotalLossAmount = new BN(0)
		let simLossAmountDeposited = new BN(0)

		let binLiquidityDistribution: BinLiquidityDistribution[] = []

		// Simulate deposit amount after loss
		for (let i = lowerBinId; i <= upperBinId; i++) {
			const amount = binDepositAmount.get(i)

			const distrubitonX = amount
				.mul(new BN(BASIS_POINT_MAX))
				.div(positionSeedAmount)

			const depositedAmount = positionSeedAmount
				.mul(distrubitonX)
				.div(new BN(BASIS_POINT_MAX))

			simAmountDeposited = simAmountDeposited.add(depositedAmount)

			binLiquidityDistribution.push({
				binId: i,
				distributionX: distrubitonX.toNumber(),
				distributionY: 0
			})

			const loss = amount.sub(depositedAmount)
			simTotalLossAmount = simTotalLossAmount.add(loss)
		}
		...
		for (let i = lowerBinId; i <= upperBinId; i++) {
			...
			const depositedLossAmount = simTotalLossAmount
				.mul(distrubitonX)
				.div(new BN(BASIS_POINT_MAX))

			simLossAmountDeposited = simLossAmountDeposited.add(depositedLossAmount)
		}
	...
	private async preparePositionAndBinArrayForSeeding(
		lowerBinId: BN,
		upperBinId: BN,
		base: PublicKey,
		position: PublicKey,
			...
			instructions.push(
				await this.program.methods
					...
					.accounts({
						...
						base,
						owner,
						ownerTokenX: getAssociatedTokenAddressSync(
							this.tokenX.publicKey,
							owner,
							true
						),
						operator,
						operatorTokenX: getAssociatedTokenAddressSync(
							this.tokenX.publicKey,
							operator
						),
						payer: operator
		...
		const lowerBinArrayState = lowerBinArrayAccount
			? this.program.coder.accounts.decode<BinArray>(
					this.program.account.binArray.idlAccount.name,
					lowerBinArrayAccount.data
				)
			: null

		const upperBinArrayState = upperBinArrayAccount
			? this.program.coder.accounts.decode<BinArray>(
					this.program.account.binArray.idlAccount.name,
					upperBinArrayAccount.data
				)
			: null
