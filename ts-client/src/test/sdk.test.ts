...
import { findSwappableMinMaxBinId, getQPriceFromId } from "../dlmm/helpers/math"
...
const keypair = Keypair.fromSecretKey(new Uint8Array(JSON.parse(keypairBuffer)))
...
describe("SDK test", () => {
	...
	describe("Permissioned lb pair", () => {
		...
		it("findSwappableMinMaxBinId returned min/max bin id are 1 bit from max/min value", () => {
			for (let binStep = 1; binStep <= 500; binStep++) {
				const { minBinId, maxBinId } = findSwappableMinMaxBinId(new BN(binStep))
		...
		it("initialize position and add liquidity buy side", async () => {
			...
			await sendAndConfirmTransaction(connection, initializePositionByOperatorTx, [
				keypair,
				baseKeypair
			]).catch((e) => {
				console.error(e)
		...
		it("update activation point", async () => {
			try {
				...
				const txHash = await sendAndConfirmTransaction(connection, rawTx, [keypair])
		...
		it("remove liquidity from position with custom owner, capital to position owner, but fee to fee owner", async () => {
			const activeBinArrayIdx = binIdToBinArrayIndex(new BN(pair.lbPair.activeId))
		...
		it("remove liquidity from position, capital and fee to position owner", async () => {
			...
			const fullAmountX = new Decimal(positionState.positionData.feeX.toString())
				.add(positionState.positionData.totalXAmount)
				.floor()

			const fullAmountY = new Decimal(positionState.positionData.feeY.toString())
	...
	describe("seed liquidity", () => {
		...
		it("Happy path", async () => {
			...
			const maxPrice = getPriceOfBinByBinId(
				pair.lbPair.activeId + 1 + MAX_BIN_PER_POSITION.toNumber() * positionNeeded,
				pair.lbPair.binStep
			).mul(priceMultiplier)
	...
	it("create LBCLMM instance with bitmap extension", async () => {
		...
		lbClmmWithBitMapExt = await DLMM.create(connection, lbPairWithBitMapExtPubkey, {
			cluster: "localhost"
		})
		expect(lbClmmWithBitMapExt).not.toBeNull()
		expect(lbClmmWithBitMapExt).toBeInstanceOf(DLMM)
