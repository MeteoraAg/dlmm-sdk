...
async function main() {
  ...
  const positions = await jlpHubSolPool.getPositionsByUserAndLbPair(walletKey);
  ...
  for (const position of positions.userPositions) {
    ...
    position.positionData.positionBinData.forEach((bin) => {
      console.log("BinId", bin.binId.toString());
      console.log(
        "FeeXY",
        bin.positionFeeXAmount.toString(),
        bin.positionFeeYAmount.toString()
      );
      console.log(
        "Reward01",
        bin.positionRewardAmount[0].toString(),
        bin.positionRewardAmount[1].toString()
      );
    });
  }
