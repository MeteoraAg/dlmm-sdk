...
def _parse_int(value: str) -> int:
    """Parse an integer value that may be in hex or decimal format."""
    if isinstance(value, int):
        return value
    value = str(value).strip()
    # Check if it's a hex string (contains a-f characters or starts with 0x)
...
class SwapQuote():
    ...
    def __init__(self, data: dict):
        self.consumed_in_amount = _parse_int(data["consumedInAmount"])
        self.out_amount = _parse_int(data["outAmount"])
        self.fee = _parse_int(data["fee"])
        self.protocol_fee = _parse_int(data["protocolFee"])
        self.min_out_amount = _parse_int(data["minOutAmount"])
