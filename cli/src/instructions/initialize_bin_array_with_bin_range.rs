...
use anyhow::*;
use dlmm_program_interface::state::bin::bin_id_to_bin_array_index;
use std::ops::Deref;
...
pub fn initialize_bin_array_with_bin_range<C: Deref<Target = impl Signer> + Clone>(
    ...
    let lower_bin_array_idx = bin_id_to_bin_array_index(lower_bin_id);
    let upper_bin_array_idx = bin_id_to_bin_array_index(upper_bin_id);
