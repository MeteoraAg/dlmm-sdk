...
fn test_claim_reward() {
    ...
    let mut position_0 = init_position(active_id);
    ...
    position_0.reset_all_pending_reward(reward_index);
    let total_reward = position_0.get_total_reward(reward_index).unwrap();
    assert_eq!(total_reward, 0);

    // other user deposit
    let mut position_1 = init_position(active_id);
    // Deposit
    deposit(
        &mut lb_pair,
        &mut position_1,
...
fn test_deposit_after_reward_duration_end() {
    ...
    let mut position = init_position(active_id);
...
fn claim_reward(
    ...
    position
        .update_earning_per_token_stored(&bin_array_manager)
        .unwrap();

    position.get_total_reward(reward_index).unwrap()
}
...
pub fn init_position(active_id: i32) -> PositionV2 {
    let lower_bin_id = active_id;
    let upper_bin_id = active_id + MAX_BIN_PER_POSITION as i32 - 1;
    PositionV2 {
        lower_bin_id,
        upper_bin_id,
...
fn assert_position_liquidity(position: &mut PositionV2, bin_id: i32, liquidity_share: u128) {
